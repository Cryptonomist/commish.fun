...
export default function Page() {
  return (
    ...
    >
      ...
      <p><strong>{"The upgrade authority is a single key that we hold."}</strong>{" The program can, in principle, be replaced with different code by whoever holds that key. Today that is one private key held by us. Our intention is to move it to a 2 of 3 multisig and then discard it entirely, so that the program becomes immutable. "}<strong>{"That has not happened yet."}</strong>{" Until it does, the guarantees described above rest on that key not being misused, lost or stolen. This is the single most material risk in the product and you should weigh it before sending anything of value."}</p>
