...
Security (each tested): canonical USDC per cluster (mainnet `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`); vault = pool ATA; SPL Token program checked; signer/owner/PDA/bump on every account; `init` only; checked math; `Clock::get()` only, boundary tests ±1s; idempotent settle/claim/refund/finalize; veto once per member per posting, strict majority; **slot assignees must be existing paid members** (prevents paying a typo'd address); prize sum enforced at creation; fee computed once, capped, never on $0 pools; `paused` blocks creation only, never claims or refunds. **Upgrade authority:** founder-held and disclosed on `/trust`, and NOT promised away. A 2-of-3 multisig within 30 days was the plan of record here and is no longer planned; what happens to the key is decided after an audit, because the audit is what says whether freezing the program is safe. Keep it and members trust one person, discard it and any unfound bug is permanent, share it and you sit between the two. No option is committed to and no date is attached. Note the consequence for §19.3: the multisig was also carrying the money-transmission "control" mitigation, so that argument now rests on the rest of the list, on disclosure, and on the fact that the key cannot reach a vault without a redeploy.
...
Zero economic benefit from real-money pools · non-custodial by construction, with the upgrade authority disclosed rather than promised away · private link-only pools, no directory or matchmaking · no betting language, odds, or lines anywhere · $0 pools available everywhere · full transparency on `/trust` and in `docs/LEGAL.md` · **no pool money ever touches a token (§3.1)**.
